...
def execute_SHIFTIWOP_pure64 (shamt : (BitVec 5)) (op : sopw) (rs1_val : (BitVec 64)) : BitVec 64 :=
  ...
  let result : (BitVec 32) :=
    match op with
    | RISCV_SLLIW => (shift_bits_left rs1_val32 shamt)
    | RISCV_SRLIW => (shift_bits_right rs1_val32 shamt)
    | RISCV_SRAIW => (shift_bits_right_arith rs1_val32 shamt)
...
def execute_SHIFTIOP_pure64 (shamt : (BitVec 6)) (op : sop) (rs1_val : (BitVec 64)) : BitVec 64 :=
      ...
      | RISCV_SRAI => (shift_bits_right_arith (rs1_val) shamt)
...
def execute_RTYPEW_pure64 (op : ropw) (rs2_val : (BitVec 64)) (rs1_val : (BitVec 64)) : BitVec 64 :=
  ...
  let result : (BitVec 32) :=
    match op with
    | RISCV_ADDW => (rs1_val32 + rs2_val32)
    | RISCV_SUBW => (rs1_val32 - rs2_val32)
    | RISCV_SLLW => (shift_bits_left rs1_val32 (Sail.BitVec.extractLsb rs2_val32 4 0))
    | RISCV_SRLW => (shift_bits_right rs1_val32 (Sail.BitVec.extractLsb rs2_val32 4 0))
    | RISCV_SRAW => (shift_bits_right_arith rs1_val32 (Sail.BitVec.extractLsb rs2_val32 4 0))
  ...
  let result :=
      ...
      | RISCV_SLL =>
        (shift_bits_left (rs1_val) --reason for log2.. etc is it extracts the lower 6 bits where the shift command is encoded
            (Sail.BitVec.extractLsb (rs2_val) (log2_xlen -i 1) 0)) --(log2_xlen -i 1) log2_xlen yields 6 thus overall extracts the last 5 bits
      | RISCV_SRL =>
        (shift_bits_right (rs1_val)
            (Sail.BitVec.extractLsb (rs2_val) (log2_xlen -i 1) 0))
...
def execute_REM_pure64 (s : Bool) (rs2_val : (BitVec 64)) (rs1_val : (BitVec 64)) : BitVec 64 :=
  ...
  (to_bits xlen r) -- converts it to xlen bits, usually 64
...
def execute_MUL_pure64 (mul_op : mul_op) (rs2_val : (BitVec 64)) (rs1_val : (BitVec 64)) : BitVec 64 :=
  ...
  let result_wide := (to_bits (2 *i xlen) (rs1_int *i rs2_int)) --adapt result vector width to 2^(exp1 + exp2)
  let result :=
    if mul_op.high -- if set return the higher xlen bits else the lower
    then (Sail.BitVec.extractLsb result_wide ((2 *i xlen) -i 1) xlen) --return either higher or lower xlen bits
    else (Sail.BitVec.extractLsb result_wide (xlen -i 1) 0)
  result
/-
...
def execute_ITYPE_pure64 (imm : (BitVec 12)) (rs1_val : (BitVec 64)) (op : iop) : BitVec 64 :=
   let immext : xlenbits := (sign_extend (m := ((2 ^i 3) *i 8)) imm) -- sign extend immediate to 64 bits
    match op with -- result value will be returned instead of written into destination register
      | RISCV_ADDI => rs1_val + immext --immediate addition
      | RISCV_SLTI =>
        zero_extend (m := ((2 ^i 3) *i 8))
